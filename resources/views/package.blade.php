    ...
    <main>
        ...
        ">
            <div class="hero-content h-100 d-flex justify-content-center align-items-center flex-column">
                <h1 class="text-center text-white display-4">Tour du lịch hấp dẫn</h1>
        ...
        <section class="container package text-center" id="package" style="margin-top: -60px">
            ...
            <ul class="pagination justify-content-center">
                <li class="page-item {{ $travelPackages->onFirstPage() ? 'disabled' : '' }}">
                    <a class="page-link" href="{{ $travelPackages->previousPageUrl() }}">
                        <span>Previous</span>
                    </a>
                </li>
                @for ($i = 1; $i <= $travelPackages->lastPage(); $i++)
                    <li class="page-item {{ $i == $travelPackages->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $travelPackages->url($i) }}">{{ $i }}</a>
                    </li>
                @endfor
                <li class="page-item {{ $travelPackages->hasMorePages() ? '' : 'disabled' }}">
                    <a class="page-link" href="{{ $travelPackages->nextPageUrl() }}">
                        <span>Next</span>
                    </a>
                </li>
            </ul>
